...
CREATE TABLE IF NOT EXISTS public.user_profiles (
    ...
    display_name            TEXT CHECK (char_length(display_name) <= 256),
    -- 온보딩 7단계 데이터 (FR-0.2)
...
CREATE INDEX IF NOT EXISTS idx_learning_paths_stuck    ON public.learning_paths(processing_started_at)
    WHERE status = 'processing';
...
-- signals / signal_sources: 플랫폼 공개 읽기 (인증 사용자 SELECT 허용, 쓰기는 service_role 전용 — AD-3)
ALTER TABLE public.signals           ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.signal_sources    ENABLE ROW LEVEL SECURITY;

-- user_profiles — SELECT only (쓰기: handle_new_user 트리거 + FastAPI service_role, AD-3)
...
BEGIN
    ...
    -- Stuck learning paths (processing_started_at 기준, AD-12)
...
-- mark_stuck_jobs: service_role만 호출 가능 (PostgREST 경유 일반 사용자 차단)
