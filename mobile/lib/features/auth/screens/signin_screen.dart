    ...
    try {
      ...
      if (userId != null && mounted) {
        final profile = await Supabase.instance.client
            ...
            .eq('id', userId)
            .maybeSingle();
        // 라우터 게이트가 참조하는 플래그를 이 사용자의 DB 값으로 동기화
        // (기기 전역 pref에 이전 사용자 값이 남아 잘못 라우팅되는 문제 방지)
        final done = profile?['onboarding_completed'] == true;
        ref.read(onboardingCompletedProvider.notifier).state = done;
        final prefs = await SharedPreferences.getInstance();
        await prefs.setBool(kOnboardingCompletedKey, done);
        if (mounted) {
